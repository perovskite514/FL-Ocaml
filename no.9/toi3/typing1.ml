...
let print_type ty =
  ...
  | TyPair (t1, t2) -> "(" ^ (type_to_string t1) ^ " , " ^ (type_to_string t2) ^ ")"
  | TyList (lst) -> "cons"
...
let rec ty_subst subst ty =  
    ...
    in match ty with
        ...
        | TyList (ty) -> TyList (ty_subst subst ty)
...
let unify cons =  
    let rec solve cons subs =
      ...
      | (_, TyVar i) -> if (occurs t2 t1) then raise Not_found
                          ...
                          solve (List.map (fun (t1, t2) -> (ty_subst s t1, ty_subst s t2)) cons2) 
                                                                (compose s subs)
      | (TyPair(t1, t2), TyPair(t3, t4)) -> solve ((t1, t3) :: (t2, t4) :: cons2) subs
      | (TyList t1, TyList t2) -> solve ((t1, t2) :: cons2) subs
	    | (_, _) -> raise Not_found
        end
  in solve cons []
