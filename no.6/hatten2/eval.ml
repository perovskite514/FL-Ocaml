...
let rec eval_expr env e =
  ...
  | EApp (e1,e2) ->
      ...
      (match v1 with 
        ...
        | _ -> raise EvalErr)
  | ELetRec (f,x,e1,e2) -> let rec v = VFun(x,e1,newenv) and newenv = ((f, v) :: env) in eval_expr newenv e2
  
let rec eval_command env c =
  match c with
  | CExp e -> ("-", env, eval_expr env e)
  | CDecl (n, e) -> let e1 = eval_expr env e in (n, extend n e1 env, e1)
  | CRecDecl (f, x, e) -> let rec v = VFun(x,e,newenv) and newenv = ((f, v) :: env) in (f, newenv, VFun(x,e,newenv))