...
*)
(* トークンの定義 *)
type token = INVALID | EQ | LEQ | LT | PLUS | IF | THEN | INT | ID | GT | GEQ | ELSE | TIMES
...
let rec main (input: string) =
   ...
   pos_start := 0; pos_current := 0; last_token := INVALID; counter := 0;
   q0()
...
and q_c() =
  match readc() with
    '*' -> (pos_start := !pos_start+1; counter := !counter+1; q_n())
  | c -> if c='\000' then (report_error(!pos_current)) else report_error(!pos_current)

and q_cs() =
  match readc() with
    '*' -> (pos_start := !pos_start+1; counter := !counter+1; q_n())
  | c -> if c='\000' then (report_error(!pos_current)) else (pos_start := !pos_start+1; q_n())

and q_ce() =
  match readc() with
    '/' -> if !counter = 1 then (pos_start := !pos_start+1; counter := !counter-1; q0()) else (pos_start := !pos_start+1; counter := !counter-1; q_n())
  | c -> if c='\000' then (report_error(!pos_current)) else (pos_start := !pos_start+1; q_n())

and q_n() =
  match readc() with
    '/' -> (pos_start := !pos_start+1; q_cs())
  | '*' -> (pos_start := !pos_start+1; q_ce())
  | c -> if c='\000' then (report_error(!pos_current)) else (pos_start := !pos_start+1; q_n())
